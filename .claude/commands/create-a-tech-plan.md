...
# Command: Architect (Implementation Planning)

## Purpose

Guide the user through creating a validated implementation plan for a feature or requirement. This command orchestrates the planning workflow, manages user interaction, and delegates heavy work to specialized subagents.
...
│                                                                  │
│  PHASE 1.5: DOMAIN MAPPING                                       │
│                                                                  │
│  0.5. Map each requirement to its product + domain              │
│       - Use decision tree from smc-domain-design.md             │
│       - Present domain mapping to user, WAIT for confirmation   │
│                                                                  │
│  PHASE 2: FOR EACH REQUIREMENT                                   │
│                                                                  │
│  1. Spawn formulate-approach agent → get approach + assumptions │
│     (pass domain mapping as context)                            │
...
**WAIT for user confirmation** before proceeding to Step 0.5.

---

### Step 0.5: Domain Mapping

After requirements are confirmed, map each requirement to its product and domain BEFORE formulating approaches. This uses the Domain Identification Decision Tree from `smc-domain-design.md`.

For each requirement from Step 0, determine:

1. **Product**: Which product (tool) does this belong to? (e.g., mmp-builder, ilf)
2. **Primary domain**: Which domain within that product? Use the decision tree and domain inventory from `smc-domain-design.md`.
3. **Cross-domain dependencies**: Does this requirement need data/services from other domains or products? Identify which services will be called and in which direction.
4. **New tables**: If the requirement needs new database tables, apply the Aggregate Ownership Test from `smc-domain-design.md` to determine which domain's aggregate the table belongs to.
5. **New domain needed?**: If a requirement does not fit any existing domain, apply the "Should this be a new domain?" checklist. If warranted, propose it to the user.

**Present the domain mapping to the user:**

```
"Domain mapping for the requirements:

| # | Requirement | Product | Domain | Cross-Domain Deps | New Tables | Notes |
|---|-------------|---------|--------|-------------------|------------|-------|
| 1 | [title] | [product] | [domain] | [domain A → domain B] | [table → domain] | |
| 2 | [title] | [product] | [domain] | none | none | |

Questions requiring your input:
- Requirement [X]: This could belong to [Domain A] or [Domain B]. Which is correct?
- Requirement [Y]: This needs a new table. Should it be part of [Domain Z]'s aggregate?

Does this domain mapping look correct?"
```

**WAIT for user confirmation** before proceeding to Step 1.

**Important**: Pass the confirmed domain mapping as context to ALL subsequent subagent calls (formulate-approach, implementation-researcher, implementation-docs-writer).

---

### Step 1: Spawn formulate-approach Agent

For each requirement (from Step 0), spawn the formulate-approach agent with the domain context from Step 0.5:
...
Task tool:
  ...
  prompt: |
    ...
    Domain context:
      Product: [product from Step 0.5]
      Domain: [domain from Step 0.5]
      Cross-domain dependencies: [from Step 0.5]
      New tables: [from Step 0.5, with aggregate ownership decisions]
...
- [ ] Did I complete domain mapping before formulating approaches? (Step 0.5)
- [ ] Did I get user confirmation on the domain mapping?
- [ ] Am I passing domain context to subagents?
...
- **Skipping domain mapping**: Don't formulate approaches without knowing the product and domain first. Always complete Step 0.5 before Step 1.
...
- **Guessing domain assignment**: If domain ownership is unclear, ASK the user. Never guess which domain a feature or table belongs to.
