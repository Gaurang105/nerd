...
# Command: Execute a Plan (Technical Program Manager)
...
**This command requires an implementation plan document.** Do NOT proceed without one.
...
- A document created via the `/create-a-tech-plan` command
...
│  4. Determine affected repos and derive branch name             │
│                                                                  │
│  PHASE 2: BUILD DEPENDENCY GRAPH                                 │
│                                                                  │
│  5. Categorize tasks: Foundational / Dependent / Independent    │
│  6. Map dependencies between tasks                              │
│  7. Create execution order                                      │
│  8. Present dependency graph + git info, WAIT for approval      │
│                                                                  │
│  PHASE 3: GIT SETUP                                              │
│                                                                  │
│  9. Checkout feature branch in affected submodule repos         │
│                                                                  │
│  PHASE 4: EXECUTE (for each task)                                │
│                                                                  │
│  10. Execute foundational tasks first (sequential)              │
│      → Commit & push after each task                            │
│  11. Execute dependent/independent tasks (parallel where safe)  │
│      → Commit & push after each task                            │
│  12. Code review on ALL changes after all waves complete        │
│      → Commit & push fixes if any                               │
│                                                                  │
│  PHASE 5: FINALIZE                                               │
│                                                                  │
│  13. Structured summary of all tasks and review results         │
└─────────────────────────────────────────────────────────────────┘
```
...
Please provide the path, or let me know if you'd like to create a plan first using the `/create-a-tech-plan` command."
```
...
#### Step 4: Determine Affected Repos and Branch Name

Using the file change map from Step 3, determine which submodule repos are affected and derive a branch name:

**Determine affected repos:**
- Files under `mmp-builder/` → frontend repo (`mmp-builder/`)
- Files under `supply-mission-control/` → backend repo (`supply-mission-control/`)
- Files may affect one or both repos

**Derive branch name from the plan document:**
- Extract feature name from the plan path (e.g., `requirements/combo-sync-implementation-plan.md` → `combo-sync`)
- Branch name format: `feat/<feature-name>`

```
Affected repos:
  - mmp-builder/ (frontend) → branch: feat/<feature-name>
  - supply-mission-control/ (backend) → branch: feat/<feature-name>
```
...
Affected repos:
  - ✅ mmp-builder/ (frontend) — branch will be created
  - ✅ supply-mission-control/ (backend) — branch will be created
...
Does this execution order and git setup look correct?"
```
...
#### Step 9: Checkout Feature Branch

After user approval, checkout the feature branch in each affected submodule repo:

```bash
# For each affected submodule repo:
cd <submodule-path>
...
- Only checkout in repos that have changes (don't touch unaffected repos)
- If the branch already exists, check it out (don't error)
...
#### Step 10: Execute Foundational Tasks
...
**After each foundational task completes → Commit & Push:**

```bash
# Identify which submodule the changed files belong to
cd <submodule-path>
...
# Push (first push sets upstream, subsequent pushes just git push)
git push -u origin feat/<feature-name>  # first time
git push                                 # subsequent
```

**Determining commit type:**
- Task touches `database/migrations/` or config → `chore`
- Task creates new features/endpoints/components → `feat`
- Task fixes bugs → `fix`
- Task refactors existing code → `refactor`

Proceed to the next foundational task. Do not run code review until all waves are complete.

#### Step 11: Execute Dependent/Independent Tasks
...
**After each agent returns → Commit & Push (sequentially, even if development was parallel):**
...
**After all waves complete**, proceed to Step 12 (code review).

#### Step 12: Code Review + Fix (After All Waves Complete)

**This step runs once after ALL tasks in ALL waves have completed.** The code review agent reviews all changes across the entire implementation.

**12a. Run code review on all changes:**

```
Task tool:
  subagent_type: code-review-analyst
  prompt: |
    Review the complete implementation of all tasks.

    Implementation plan: requirements/<feature>-implementation-plan.md

    ## All Tasks and Files Changed
    [For each task, list:
    - Task N: [task title]
      - Files: [list of files]
    ...
    ## Full list of files changed across all tasks
    - [consolidated list of all files from all tasks]

    Provide:
    1. Code quality comments (blockers, major issues, suggestions, nits) for the entire implementation
    2. Feature completeness check against acceptance criteria for each task
    3. Acceptance criteria match: for each task, list each criterion as MET or NOT MET
```

**12b. If blockers or major issues found → fix immediately:**

```
Task tool:
  subagent_type: developer-agent
  prompt: |
    Fix code review issues across the implementation.
...
**After fixes → Commit & Push the review fixes:**

```bash
# For each affected submodule repo with fixes:
cd <submodule-path>
git add <fixed-files>
...
**12c. Fix-Review Loop (after all waves):**

```
┌─────────────────────────────────────────────────────────┐
│  Execute all waves (Steps 10 + 11)                      │
│  developer-agents implement all tasks                   │
│  commit & push after each task                          │
│              ↓                                          │
│  code-review-analyst reviews ALL changes (12a)          │
│              ↓                                          │
│  Has blockers/major issues?                             │
│     YES → developer-agent fixes (12b)                   │
│           → commit & push fixes                         │
│           → re-review ALL changes (back to 12a)         │
│     NO  → implementation is DONE, proceed to summary    │
└─────────────────────────────────────────────────────────┘
```

**Important**:
- Code review runs **only after all waves complete** - do not run per-task reviews
- Blockers AND majors are **always fixed** before finalizing
...
- Every commit must be followed by a push

---

### Phase 5: Finalize

#### Step 13: Structured Summary

Present the final structured summary to the user. This is the primary output of the command.
...
| `code-review-analyst` | After all waves complete (and after each fix) | Review comments for all changes, acceptance criteria match per task |
...
- **Skipping reviews**: Code review must run on all changes after all waves complete
- **Ignoring blockers**: Fix all blockers found in the final code review before finalizing
...
- **Committing to wrong repo**: Always verify which submodule the changed files belong to before staging and committing
- **Forgetting to push**: Every commit must be followed by a push to the remote branch
