...
**Domain**: [Which product and domain from the SMC Domain Inventory this requirement touches.
Use the Domain Identification Decision Tree from smc-domain-design.md.
If it spans multiple domains, list the primary domain first, then others.
If unclear, write "UNCLEAR — needs user input" and explain why.]
...
For each requirement, tag the product and domain using the Domain Inventory from `smc-domain-design.md`:
- If a requirement clearly belongs to one domain, tag it (e.g., "mmp-builder / Assortment").
- If it spans multiple domains, tag the primary domain first, then list others (e.g., "mmp-builder / Content Generation, mmp-builder / Scraping").
- If you cannot determine the domain, tag it "UNCLEAR — needs user input" and note why.
- Flag any requirement that appears to span 3 or more domains — this likely needs decomposition into separate requirements.
