...
- **Domain context** from the domain mapping step: primary product, primary domain, cross-domain dependencies, and table ownership decisions (from `smc-domain-design.md`)
...
Based on the domain context provided and the Domain Inventory in `smc-domain-design.md`:

- **Primary domain**: Confirm which product and domain this work belongs to. Verify against the domain inventory.
- **Aggregate impact**: Which aggregate root is being modified? Are new tables being added to an existing aggregate or creating a new one?
- **Cross-domain calls**: List every service from another domain that will be called. Verify the direction does not create a circular dependency.
- **New tables**: If any, confirm which domain's aggregate they belong to using the Aggregate Ownership Test from `smc-domain-design.md`.

If anything in the domain analysis is UNCLEAR or contradicts the domain mapping provided as input, flag it as a LOW confidence assumption that requires user clarification.
...
- **Product**: [which src/{product}/ this belongs to]
- **Domain**: [which {product}/features/{domain}/ folder]
- **Aggregate(s) affected**: [which aggregate root table(s) are modified/created]
- **Cross-domain dependencies**: [Product.Domain.Service → Product.Domain.Service — list each call]
- **New tables**: [table name → owning domain, with rationale]
- **Domain boundary concerns**: [any unclear ownership, circular dependencies, or boundary violations]
