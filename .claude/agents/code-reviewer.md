...
### Discover project-specific rules

1. Determine which repo/submodule the changed files belong to
2. Check for `CLAUDE.md` at the repo root
3. Check for `.claude/rules/` at the repo root
4. If either exists, read all of them — these are mandatory coding standards
5. If changes span multiple repos, discover rules for each separately
...
- **Performance**: N+1 queries, unnecessary loops, blocking calls in async contexts, missing caching opportunities, O(n²) where O(n) is possible
  - **N+1 queries**: `for`/`map`/`forEach` containing `await` calls to repos or gateway clients — should be a single batch call outside the loop
  - **Sequential awaits that could be parallel**: Independent `await` calls in sequence that should use `Promise.all`
  - **Unbounded queries**: Fetching all rows without `LIMIT` or pagination
...
- **Error handling**: Swallowed errors, missing cleanup in error paths, unhelpful error messages, missing timeouts on external calls
  - **Swallowed errors**: Empty `catch` blocks or `catch` that only logs but doesn't rethrow/return error
  - **Missing timeouts**: External HTTP calls or DB queries without explicit timeout config
...
- **Type safety**: Unsafe casts, `any` types, unhandled null cases
  - `as` casts bypassing the compiler instead of narrowing via type guards
  - Optional chain (`?.`) without handling the `undefined` result downstream
- **Patterns**: Deviations from existing codebase conventions, duplicated logic, tight coupling
  - Cross-reference against project rules discovered in Phase 1
  - Check if new code reimplements something that already exists in `shared/` or `utils/`
...
If project rules were discovered in Phase 1, check whether changed code violates them:

1. For each rule file, check whether changed code violates it. Focus on:
   - Architecture/layer rules (e.g., controller calling repo directly)
   - Naming conventions
   - Error handling rules (e.g., plain `Error` instead of custom classes)
   - Folder/file placement rules
2. Write review comments using the standard format above. Severity: **major** for architectural violations, **suggestion** for naming/style.
3. Cite the specific rule being violated (e.g., "Violates `smc-error-handling` rule: all errors must be custom error classes").
4. Only flag violations in changed code, not pre-existing issues.
...
### Project Rules: X violations found (Y major, Z suggestions)
