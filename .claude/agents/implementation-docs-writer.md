...
Every implementation document MUST include a domain context section that maps the work to SMC's domain structure (see `smc-domain-design.md` for the full domain identification decision tree and inventory).

- **Product**: Which `src/{product}/` this work belongs to (e.g., mmp-builder, ilf).
- **Domain**: Which `{product}/features/{domain}/` folder this work lives in.
- **Aggregate**: Which aggregate root table is the primary entity being affected.
- **Tables affected**:
  | Table | Action | Owning Domain |
  |-------|--------|---------------|
  | [table_name] | CREATE/MODIFY/READ | [product / domain] |
- **Cross-domain service calls**:
  | Caller (Product.Domain.Service) | Callee (Product.Domain.Service) | Purpose |
  |----------------------------------|----------------------------------|---------|
  | [caller] | [callee] | [why] |
- **Domain boundary compliance**:
  - [ ] No repository imports across domain boundaries
  - [ ] All cross-domain access goes through services
  - [ ] New tables are assigned to the correct aggregate per domain rules
...
1. All seven required sections are present and complete.
...
7. The Domain Context section is present and accurately maps all tables and cross-domain calls.
